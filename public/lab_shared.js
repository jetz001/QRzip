    ...
    clearBtn.addEventListener("click", () => {
      ...
      if(qrPayloadView) qrPayloadView.textContent = "ยังไม่มีข้อมูล";
      if(qrDecodeFlowView) qrDecodeFlowView.textContent = "ยังไม่มีข้อมูล";
      lastBestResult = null;
      lastResults = [];
    });
    ...
    if(generateMemberQrBtn) generateMemberQrBtn.addEventListener("click", generateMemberQrCode);
    if(resolveRefBtn) resolveRefBtn.addEventListener("click", resolveMemberRef);
    ...
    function runCompression() {
      ...
      rawQrBox.textContent = "ยังไม่มี QR";
      compressedQrBox.textContent = "ยังไม่มี QR";
      if(qrPayloadView) qrPayloadView.textContent = "กดปุ่มสร้าง QR เทียบกันเพื่อดู payload";
      if(qrDecodeFlowView) qrDecodeFlowView.textContent = "กดปุ่มสร้าง QR เทียบกันเพื่อดูว่า decoder ควรรับข้อมูลตัวไหน";
      if(memberQrMeta) memberQrMeta.textContent = "กดปุ่มสร้าง QR สมาชิก (สั้นสุด)";
      if(memberQrBox) memberQrBox.textContent = "ยังไม่มี QR";
      if(refResultView) refResultView.textContent = "ยังไม่มีข้อมูล";
    }
    ...
    function generateQrCode() {
      ...
      if (typeof QRCode === "undefined") {
        ...
        if(qrDecodeFlowView) qrDecodeFlowView.textContent = "โหลดไลบรารี QR ไม่สำเร็จ";
        return;
      }
    ...
    async function generateMemberQrCode() {
      if (!lastBestResult) {
        if(memberQrMeta) memberQrMeta.textContent = "ยังไม่มีผลวิเคราะห์";
        if(memberQrBox) memberQrBox.textContent = "ยังไม่มี QR";
        return;
      }

      if(memberQrMeta) memberQrMeta.textContent = "กำลังบันทึกข้อมูลไปหลังบ้าน...";
      if(refResultView) refResultView.textContent = "ยังไม่มีข้อมูล";
      ...
      try {
        ...
        if(memberQrMeta) memberQrMeta.textContent = `payload ${bytes.toLocaleString()} bytes | ref ${id}${render.mode === "chunked" ? ` | ${render.chunkCount} QR` : ""}`;
        refInput.value = ref;
        qrStatus.textContent = `สร้าง QR สมาชิกแล้ว | ref สั้นมาก (${bytes.toLocaleString()} bytes)`;
      } catch (error) {
        if(memberQrMeta) memberQrMeta.textContent = "สร้าง QR สมาชิกไม่ได้ (ต้องรัน qrzip_server.py)";
        if(memberQrBox) memberQrBox.textContent = "ยังไม่มี QR";
      }
    }
    ...
    async function resolveMemberRef() {
      ...
      if (!rid) {
        if(refResultView) refResultView.textContent = "กรุณาวาง QZR|<id> หรือ <id>";
        return;
      }
      if(refResultView) refResultView.textContent = "กำลังดึงข้อมูล...";
      try {
        const response = await fetch(`/api/get/${encodeURIComponent(rid)}`);
        const data = await response.json();
        if (!response.ok) throw new Error(data?.error || "get_failed");
        if(refResultView) refResultView.textContent = data.text || "";
      } catch (error) {
        if(refResultView) refResultView.textContent = "ดึงข้อมูลไม่ได้ (ต้องรัน qrzip_server.py และใช้ลิงก์ผ่านเซิร์ฟเวอร์)";
      }
    }
