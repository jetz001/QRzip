...
function buildHuffmanTree(freqMap) {
  ...
  while (nodes.length > 1) {
    nodes.sort((a, b) => {
      var _a, _b;
      return a.freq - b.freq || String((_a = a.char) != null ? _a : "").localeCompare(String((_b = b.char) != null ? _b : ""));
    });
...
function decodeFixedFromBytes(profileKey, bitLength, dataBytes) {
  const map = presetFrequencyMaps[profileKey == null ? void 0 : profileKey.toLowerCase()];
  if (!map)
    throw new Error("unknown_profile");
  const tree = buildHuffmanTree(map);
  if (!tree)
  ...
  for (let s = 0; s < symCount; s++) {
    ...
    for (; ; ) {
      if (high <= HALF) {
        ...
        low = low - HALF << 1;
        high = high - HALF << 1;
        val = val - HALF << 1 | readBit();
      } else if (low >= QTR && high <= 3 * QTR) {
        low = low - QTR << 1;
        high = high - QTR << 1;
        val = val - QTR << 1 | readBit();
      } else
        break;
    }
  }
  ...
  while (position < input.length) {
    ...
    for (let bit = 0; bit < 8 && position < input.length; bit++) {
      ...
      if (isLiteral) {
        ...
        if (start < 0)
          throw new Error("invalid_lz_distance");
...
function compressedPMatching(compressedBytes, pattern) {
...
async function decodeQrzipPayload(payload, apiGet = null) {
  if (!payload)
  ...
  if (payload.startsWith("QZ1K")) {
    ...
    if (match) {
      if (!globalDictDecodeBytes)
        throw new Error("engine_not_loaded");
  ...
  if (payload.startsWith("QZ1") && payload.length >= 4 && payload[3] !== "|") {
    ...
    if (method === "K") {
      if (!globalDictDecodeBytes)
        throw new Error("engine_not_loaded");
      if (bytes.length < 5)
        throw new Error("invalid_dict_frame");
    ...
    if (method === "F") {
      if (bytes.length < 7)
        throw new Error("invalid_fixed_frame");
    ...
    if (method === "A") {
      if (bytes.length < 8)
        throw new Error("invalid_arithmetic_frame");
  ...
  if (payload.startsWith("QZ1|G|")) {
    ...
    const tableJson = decoder.decode(safeInflate(tableDeflateBytes));
    const codebook = JSON.parse(tableJson);
...
export {
  ...
  binaryStringToBytes,
  compressedPMatching,
  decodeFixedFromBytes,
  decodeFixedPayload,
  decodeLzCompactFromBytes,
  decodeLzCompactPayload,
  decodeQrzipPayload,
  fixedIdToProfile
};
